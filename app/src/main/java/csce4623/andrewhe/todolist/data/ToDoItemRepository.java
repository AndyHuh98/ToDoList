package csce4623.andrewhe.todolist.data;
...
import csce4623.andrewhe.todolist.util.AppExecutors;
...
public class ToDoItemRepository implements ToDoListDataSource {
    ...
    public void deleteToDoItem(@NonNull final ToDoItem toDoItem) {
        ...
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                final Integer deleteSuccess = mContext.getContentResolver().delete(Uri.parse("content://" + ToDoProvider.AUTHORITY + "/" + ToDoProvider.TODOITEM_TABLE_NAME + "/" + toDoItem.getId()), "", new String[0]);
            }
        };
        mAppExecutors.diskIO().execute(runnable);
    }
